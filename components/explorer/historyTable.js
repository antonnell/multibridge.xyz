...
export default function EnhancedTable({ swapHistory }) {
  ...
  const addressClicked = (row, direction) => {
    if(direction === 'from') {
      window.open(`${row.fromChain?.explorer}/${row.fromChain?.transactionSuffix}/${row.txid}`, '_blank')
    } else {
      window.open(`${row.toChain?.explorer}/${row.toChain?.transactionSuffix}/${row.swaptx}`, '_blank')
    }
  }
